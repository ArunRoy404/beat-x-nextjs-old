...
export function DashboardNavbar({
    ...
    return (
        <header
            ...
            className={cn(
                "w-full flex items-center justify-between px-4 py-4 md:px-8 md:py-5 text-whitetext gap-4",
                className
            )}
            {...props}
        >
            ...
            <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <SidebarTrigger className="text-whitetext bg-search-btn-bg hover:bg-secondary/20 hover:text-secondary rounded-[8px] h-9 w-9 shrink-0 cursor-pointer" />
                <div className="flex flex-col min-w-0">
                    <h1
                        ...
                        className="text-whitetext font-medium truncate text-lg sm:text-2xl"
                    >
                        {title}
                    </h1>
                    {/* Subtitle is hidden on smaller screens to prevent layout crampedness */}
                    <p className="text-whitetext/60 text-xs md:text-sm mt-1 hidden sm:block truncate leading-normal">
                        {subtitle}
                    </p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 md:gap-4 shrink-0">
                ...
                >
                    ...
                    {notificationCount > 0 && (
                        <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-error text-[10px] font-bold text-whitetext leading-none">
                            {notificationCount}
                        </span>
                    )}
                </button>
                ...
                <Avatar className="w-10 h-10 border border-button-text">
                    ...
                    <AvatarFallback className="bg-search-bg/80 text-whitetext font-medium text-sm">
